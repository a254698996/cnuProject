...
CREATE TABLE IF NOT EXISTS `cnu`.`t_exchange_group` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_t_exchange_group_t_user1_idx` (`t_user_id` ASC),
  CONSTRAINT `fk_t_exchange_group_t_user1`
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_exchange_order` (
  ...
  `t_exchange_group_id_a` INT NOT NULL,
  `t_exchange_group_id_b` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_exchange_intention` (
  ...
  `t_exchange_order_id` INT NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_user` (
  ...
  `memo` VARCHAR(200) NULL,
  `t_exchange_intention_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_t_user_t_exchange_intention1_idx` (`t_exchange_intention_id` ASC),
  CONSTRAINT `fk_t_user_t_exchange_intention1`
    FOREIGN KEY (`t_exchange_intention_id`)
    REFERENCES `cnu`.`t_exchange_intention` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_goods` (
  `id` INT NOT NULL,
  `name` VARCHAR(45) NULL,
  `t_exchange_group_id` INT NOT NULL,
  `t_goods_category_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_t_goods_t_exchange_group_idx` (`t_exchange_group_id` ASC),
  INDEX `fk_t_goods_t_goods_category1_idx` (`t_goods_category_id` ASC),
  CONSTRAINT `fk_t_goods_t_exchange_group`
    FOREIGN KEY (`t_exchange_group_id`)
    REFERENCES `cnu`.`t_exchange_group` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_t_goods_t_goods_category1`
    FOREIGN KEY (`t_goods_category_id`)
    REFERENCES `cnu`.`t_goods_category` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_goods_pic` (
  ...
  `t_goods_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_t_goods_pic_t_goods1_idx` (`t_goods_id` ASC),
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_menu_has_t_menu_role` (
  ...
  `t_menu_role_id` INT NOT NULL,
  PRIMARY KEY (`t_menu_id`, `t_menu_role_id`),
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_menu_role_has_t_role` (
  ...
  PRIMARY KEY (`t_menu_role_id`, `t_role_id`),
...
CREATE TABLE IF NOT EXISTS `cnu`.`t_role_has_t_admin` (
  `t_role_id` INT NOT NULL,
  `t_admin_id` INT NOT NULL,
  PRIMARY KEY (`t_role_id`, `t_admin_id`),
