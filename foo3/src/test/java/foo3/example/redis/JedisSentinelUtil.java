...
public class JedisSentinelUtil {
	...
	private static final Logger log = LoggerFactory.getLogger(JedisSentinelUtil.class);
	...
	private static JedisSentinelUtil instance;

	private static JedisSentinelPool jedisPool;

	private static ReentrantLock lock = new ReentrantLock();

	private JedisSentinelUtil() {
	}

	public static JedisSentinelUtil getInstance() {
		if (instance == null) {
			lock.lock();
			if (instance == null) {
				instance = new JedisSentinelUtil();
			}
			lock.unlock();
		}
		return instance;
	...
	static {
		try {
			...
			InputStream input = JedisSentinelUtil.class.getResourceAsStream("redisSentinel.properties");
			prop.load(new BufferedInputStream(input));
			String AUTH = prop.getProperty("AUTH");
			int MAX_TOTAL = Integer.valueOf(prop.getProperty("MAX_TOTAL"));
			int MAX_IDLE = Integer.valueOf(prop.getProperty("MAX_IDLE"));
			int MAX_WAIT = Integer.valueOf(prop.getProperty("MAX_WAIT"));
			int TIMEOUT = Integer.valueOf(prop.getProperty("TIMEOUT"));
			int DEFAULT_EXPIRE = Integer.valueOf(prop.getProperty("DEFAULT_EXPIRE"));
			...
			if (StringUtils.isNotBlank(AUTH)) {
				jedisPool = new JedisSentinelPool(clusterName, sentinels, config, AUTH);
			} else {
				jedisPool = new JedisSentinelPool(clusterName, sentinels, config);
			}
		} catch (Exception e) {
			log.error("初始缓存连接：" + e.getMessage());
		}
	}
	...
	private void initJedisPool() {
		try {
			properties.load(new FileInputStream("reidsConfig.properties"));
		} catch (IOException e) {
		...
		int MAX_TOTAL = Integer.valueOf(properties.getProperty("MAX_TOTAL"));
		int MAX_IDLE = Integer.valueOf(properties.getProperty("MAX_IDLE"));
		int MAX_WAIT = Integer.valueOf(properties.getProperty("MAX_WAIT"));
		int TIMEOUT = Integer.valueOf(properties.getProperty("TIMEOUT"));
		int DEFAULT_EXPIRE = Integer.valueOf(properties.getProperty("DEFAULT_EXPIRE"));
		int MAXATTEMPTS = Integer.valueOf(properties.getProperty("MAXATTEMPTS"));
		...
		String clusterName = "mymaster";
		if (org.apache.commons.lang.StringUtils.isNotBlank(AUTH)) {
			jedisPool = new JedisSentinelPool(clusterName, sentinels, config, AUTH);
		} else {
			jedisPool = new JedisSentinelPool(clusterName, sentinels, config);
		}
