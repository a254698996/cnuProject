        ...
        bitmapCutter: function (settings) {
            ...
            function irotate(angle) {
                ...
                //Use 'rel' but not 'src',  this helps to make sure that the 'src' attribute was not start with 'http://localhost:8888/...'(msie)
                  src = img.attr('rel');
                $.ajax({
                    url: '/User/BitmapScissors',
                    dataType: 'json',
                    type: "POST",
                    data: { actions: 'RotateBitmap', src: src, angle: angle, t: Math.random() },
                    error: function (msg) {
                        alert('rotate failed!');
                    },
            ...
            //.css('margin', '0px 55px');
            //                                        .css('margin-left', (ps.holderSize.width - 35 * 7 + 3) / 2);

            //informations of bitmap
            var $info = $('<div title="Size Of Bitmap" class="jquery-bitmapcutter-info" style="height:0px;" />')
                                .insertAfter($opts)
                                    .css('width', ps.holderSize.width);

            //cutter
            var $cutter = $('<div class="jquery-bitmapcutter-cutter" id="cutterDiv"  style="filter:alpha(opacity=50);opacity:0.5;">&nbsp</div>')
                                    .css(ps.cutterSize)
                                        .css({
                ...
                $generate.click(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            /*{msg:'success',src:'/UploadFolder/icon/06343e3150594349bcdcbec4e5ae2f25.png'}*/
                            json = [];
                            eval("json=" + msg);
